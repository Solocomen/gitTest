...
@Service
public class EpcGenSAHandler2 {
    private final Logger logger = LoggerFactory.getLogger(EpcGenSAHandler2.class);
    ...
    public EpcGenSAHandler2(DataSource epcDataSource, DataSource fesDataSource, EpcSecurityHelper epcSecurityHelper,  EpcSaRemarkHandler epcSaRemarkHandler, EpcTncHandler epcTncHandler, EpcDocumentHandler epcDocumentHandler) {
...
//        String saPath = "https://epcdev-sales.smartone.com/api/salesDocument/sa/" + orderId + "?signId=" + signId;
    ...
    public String genSaHtml(int orderId, String signId) {
        ...
        String tmpProductLinkDesc = "";
        ArrayList<String> tmpProductCodeList = new ArrayList<>();
        HashMap<String, String> tmpProductAttachUrlMap = new HashMap<>();
        String tmpImei = "";
        ...
        StringBuilder saHtmlSB = new StringBuilder(2 * 1024); // default 2M
        String htmlPageHeader = "";
        String htmlPageFooter = "";
        String htmlContentHeader = ""; // will exist again across pages
        String htmlPersonalInfo = "";
        String htmlProductInfo = "";
        String html1638Info = "";
        String htmlContractInfo = "";
        String htmlAutopayInfo = "";
        String htmlUpfrontPaymentInfo = "";
        String htmlRemark = "";
        String htmlRemark2 = "";
        String htmlTNC = "";
        String htmlContentLogoNSign = "";
        String htmlPagebreak = "";
        int productCnt = 0;
        int nonProductCnt = 0;
        int contractCnt = 0;
        int appleCareCnt = 0;
        int rowCnt = 0;
        int offerRemarkCnt = 0;
        int offerTncCnt = 0;
        String htmlOfferTncSaRemark;
        ArrayList<String> offerRemarkList = new ArrayList<String>();
        ArrayList<EpcOfferTnc> offerTncList = new ArrayList<EpcOfferTnc>();
        String offerRemarkTemplate = "";
        String offerTncTemplate = "";
        String tmpOfferRemark = "";
        String offerTncString = "";
        StringBuilder offerRemarkSectionSB = new StringBuilder();
        ...
        try {
            ...
            // get products
            sql = "select item_id, cpq_item_desc, cpq_item_desc_chi, cpq_item_value, stock_status, item_code " +
                  "  from epc_order_item " +
                  " where order_id = ? " + 
                  "   and item_cat in (?,?,?,?,?,?) " +
                  " order by case_id ";
            ...
            rsetNonProductCharge.beforeFirst();
            ...
            // get SA offer remark
            offerRemarkList = epcSaRemarkHandler.getOrderSaRemarkForSA(conn, orderId, orderLang);
            offerRemarkCnt = offerRemarkList.size();
            
            // get offer TNC
            offerTncList = epcTncHandler.getOrderOfferTncForSA(conn, orderId);
            offerTncCnt = offerTncList.size();

            // get template
            htmlPageHeader = getTemplate(conn, "EPC_SA_FORM2_PAGE_HEADER", orderLang);
            htmlPageFooter = getTemplate(conn, "EPC_SA_FORM2_PAGE_FOOTER", orderLang);
            htmlContentHeader = getTemplate(conn, "EPC_SA_FORM2_CONTENT_HEADER", orderLang);
            htmlPersonalInfo = getTemplate(conn, "EPC_SA_FORM2_PERSONAL_INFO", orderLang);
            htmlProductInfo = getTemplate(conn, "EPC_SA_FORM2_PRODUCT_INFO", orderLang);
            htmlOfferTncSaRemark = getTemplate(conn, "EPC_SA_FORM2_TNC_AND_SA_REMARK", orderLang);
            html1638Info = getTemplate(conn, "EPC_SA_FORM2_1638_INFO", orderLang);
            htmlContractInfo = getTemplate(conn, "EPC_SA_FORM2_CONTRACT_INFO", orderLang);
            htmlAutopayInfo = getTemplate(conn, "EPC_SA_FORM2_AUTOPAY_INFO", orderLang);
            htmlUpfrontPaymentInfo = getTemplate(conn, "EPC_SA_FORM2_UPFRONT_PAYMENT_INFO", orderLang);
            htmlRemark = getTemplate(conn, "EPC_SA_FORM2_REMARK", orderLang);
            htmlRemark2 = getTemplate(conn, "EPC_SA_FORM2_REMARK2", orderLang);
            htmlTNC = getTemplate(conn, "EPC_SA_FORM2_T_N_C", orderLang);
            htmlContentLogoNSign = getTemplate(conn, "EPC_SA_FORM2_CONTENT_LOGO_N_SIGN", orderLang);
            htmlPagebreak = getTemplate(conn, "EPC_SA_FORM2_PAGEBREAK", orderLang);


//            saHtml = getTemplate(conn, orderLang);
            // append to string saHtml
            //  include determination of page break
            if(contractCnt == 0) {
                contractCnt = 1; // dummy set of rows
            }
            //rowCnt = (1 * productCnt) + (1 * nonProductCnt) + (4 * contractCnt) + (2 * appleCareCnt);
            rowCnt = (1 * productCnt) + (1 * nonProductCnt) + (4 * contractCnt) + (2 * appleCareCnt) + (2 * offerRemarkCnt) + (1 * offerTncCnt);

            if(rowCnt > 15 && rowCnt <= 20) {
                // page 1
                saHtmlSB.append(htmlPageHeader);
                saHtmlSB.append(htmlContentHeader);
                saHtmlSB.append(htmlPersonalInfo);
                saHtmlSB.append(htmlProductInfo);
                saHtmlSB.append(htmlOfferTncSaRemark);
                saHtmlSB.append(html1638Info);
                saHtmlSB.append(htmlContractInfo);
                saHtmlSB.append(htmlAutopayInfo);
                saHtmlSB.append(htmlUpfrontPaymentInfo);
                saHtmlSB.append(htmlRemark);
                // end of page 1

                // page 2
                saHtmlSB.append(htmlPagebreak);
                saHtmlSB.append(htmlContentHeader);
                saHtmlSB.append(htmlRemark2);
                saHtmlSB.append(htmlTNC);
                saHtmlSB.append(htmlContentLogoNSign);
                // end of page 2

                saHtmlSB.append(htmlPageFooter);
            } else if (rowCnt > 20) {
                // page 1
                saHtmlSB.append(htmlPageHeader);
                saHtmlSB.append(htmlContentHeader);
            ...
            offerRemarkTemplate = getTemplate(conn, "EPC_SA_FORM2_OFFER_REMARK", orderLang);
            offerTncTemplate = getTemplate(conn, "EPC_SA_FORM2_OFFER_TNC", orderLang);
            // end of get template
...
//            }
            ...
            // loop through product code list and retrieve attachment URL from FES
            while (rsetProduct.next())
            {
                tmpProductCode = StringHelper.trim(rsetProduct.getString("item_code"));

                if(!tmpProductCode.isBlank())
                {
                    tmpProductCodeList.add(tmpProductCode);
                }
            }
            rsetProduct.beforeFirst();
            tmpProductCodeList = new ArrayList<>(new HashSet<>(tmpProductCodeList));
            tmpProductAttachUrlMap = getProductAttachUrlMap(fesConn, orderLang, tmpProductCodeList);
            ...
            while(rsetProduct.next()) {
                ...
                {
                    if(!tmpProductCode.isBlank() && tmpProductAttachUrlMap.containsKey(tmpProductCode))
                    {
                        tmpProductLinkDesc = "<a href='" + tmpProductAttachUrlMap.get(tmpProductCode) + "'>" + tmpProductDesc + "</a>";
                    }
                    else
                    {
                        tmpProductLinkDesc = tmpProductDesc;
                    }
                }
                else
                {
                    if(!tmpProductCode.isBlank() && tmpProductAttachUrlMap.containsKey(tmpProductCode))
                    {
                        tmpProductLinkDesc = "<a href='" + tmpProductAttachUrlMap.get(tmpProductCode) + "'>" + tmpProductDescChi + "</a>";
                    }
                    else
                    {
                        tmpProductLinkDesc = tmpProductDescChi;
                    }
                }
                tmpProductSection = tmpProductSection.replace("[PRODUCT_DESC]", tmpProductLinkDesc);
            ...
            // replace sa remark
            // loop offer remark
            for(String offerRemark:offerRemarkList) {
                tmpOfferRemark = offerRemarkTemplate.replace("[OFFER_REMARK]", offerRemark);
                offerRemarkSectionSB.append(tmpOfferRemark);
            }
            if (offerRemarkList.size() > 0) {
                saHtml = saHtml.replace("[NO_OFFER_REMARK]", "");
                saHtml = saHtml.replace("[OFFER_REMARK_SECTION]", offerRemarkSectionSB.toString());
            } else {
                saHtml = saHtml.replace("[NO_OFFER_REMARK]", "N/A");
                saHtml = saHtml.replace("[OFFER_REMARK_SECTION]", "");
            }
            ...
            // replace offer tnc
            if (offerTncList.size() > 0) {
                offerTncString = getTncString(offerTncList, orderLang);
                saHtml = saHtml.replace("[OFFER_TNC_SECTION]", offerTncTemplate);
                saHtml = saHtml.replace("[OFFER_TNCS]", offerTncString);
            } else {
                saHtml = saHtml.replace("[OFFER_TNC_SECTION]", "");
            }
            // end of replace offer tnc
...
//logger.info("{}{}", logStr, saHtml);
    ...
    public String getProductDisplay(Connection conn) {
        ...
        try {
            sql = "select msg_content  " +
                  ...
                  "   and status = ? ";
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, "EPC_SA_FORM2_PRODUCT"); // msg_type
    ...
    public String getTncString(ArrayList<EpcOfferTnc> epcOfferTncList, String orderLang) {

        StringBuilder offerTncSB = new StringBuilder();
        for (int i=0; i < epcOfferTncList.size(); i++) {
            if (i > 0) {
                offerTncSB.append(", ");
            }
            if (orderLang.equals("C")) {
                offerTncSB.append("<a href='" + epcOfferTncList.get(i).getTncUrlZhHk() + "'>"+ epcOfferTncList.get(i).getTncNumber() + "</a>");
            } else { // default lang = ZH_HK
                offerTncSB.append("<a href='" + epcOfferTncList.get(i).getTncUrlEn() + "'>"+ epcOfferTncList.get(i).getTncNumber() + "</a>");
            }
        }
        return offerTncSB.toString();
    }
    
    public HashMap<String, String> getProductAttachUrlMap(Connection fesConn, String orderLang, ArrayList<String> productCodeList)
    {
        String tmpProductCode = "";
        String tmpAttachUrl = "";
        HashMap<String, String> productAttachUrlMap = new HashMap<>();
        PreparedStatement pstmt = null;
        ResultSet rset = null;
        String sql = "";
        ...
        try
        {
            sql = String.format(" SELECT "
                    + ("E".equals(orderLang) ? "hdr.invattach_url," : "hdr.invattach_url_chi,")
                    + "    dtl.invattach_code"
                    + " FROM "
                    + "    invattach_config_dtl dtl,"
                    + "    invattach_config_hdr hdr"
                    + " WHERE "
                    + "        dtl.invattach_code in (%s)"
                    + "    AND hdr.logid = dtl.logid ",
                    productCodeList.stream()
                            .map(v -> "?")
                            .collect(Collectors.joining(", ")));
            pstmt = fesConn.prepareStatement(sql);
            for (int i = 1; i <= productCodeList.size(); i++)
            {
                pstmt.setString(i, productCodeList.get(i - 1));
            }
            rset = pstmt.executeQuery();
            while (rset.next())
            {
                tmpProductCode = StringHelper.trim(rset.getString("invattach_code"));
                if ("E".equals(orderLang))
                {
                    tmpAttachUrl = StringHelper.trim(rset.getString("invattach_url"));

                }
                else
                {
                    tmpAttachUrl = StringHelper.trim(rset.getString("invattach_url_chi"));
                }

                productAttachUrlMap.put(tmpProductCode, tmpAttachUrl);
            }
            rset.close();
            pstmt.close();
        }
        ...
        {
            e.printStackTrace();
            logger.error("Failed to getProductAttachUrlMap, ", e);
        }
        finally
        {
            try
            ...
            {
                fe.printStackTrace();
                logger.error("Failed to close resultset, ", fe);
            }
            try
            {
                if (pstmt != null)
            ...
            {
                fe.printStackTrace();
                logger.error("Failed to close prepared statement, ", fe);
            }
        }

        return productAttachUrlMap;
    }
}
